#!/bin/bash
# this script is intended to be run from within each student's lab4/ directory. It should be copied there.
LABN=lab4

NUM_TESTS=4


declare -a file_list=("br_same.x" "cancel_req.x" "ld.x" "st_loop.x")
#declare -a file_list=("br_same.x" "cancel_req.x" "ld.x")

#declare -a file_list=("ld.x")
#declare -a file_list=("st_loop.x")
...
do
	...
	echo "go\n rdump" | timeout 3 ./sim-prints inputs/${inputfile} > reference_halt.txt
	echo "go\n rdump" | timeout 3 src/sim inputs/${inputfile} > actual_halt.txt
	...
	while [ $i -eq $value ]
		do
			# echo $cycle
			echo "run $cycle\n rdump\n mdump 0x10000000 0x100000ff" | timeout 3 ./sim-prints inputs/${inputfile} > reference.txt
			cd src
